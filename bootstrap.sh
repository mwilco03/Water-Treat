...
check_build_deps() {
    ...
    # Check for p-net (PROFINET stack) - optional but required for controller communication
    check_pnet_installed

    return 0
}

check_pnet_installed() {
    local pnet_lib="/usr/local/lib/libpnet.so"
    local pnet_header="/usr/local/include/pnet_api.h"

    if [[ -f "$pnet_lib" && -f "$pnet_header" ]]; then
        log_info "p-net PROFINET stack found"
        return 0
    fi

    log_info "Installing p-net PROFINET stack..."
    install_pnet || {
        log_warn "p-net installation failed - PROFINET will be disabled"
        return 1
    }
    return 0
}
...
install_pnet() {
    local pnet_dir="/tmp/p-net-build-$$"
    local pnet_repo="https://github.com/rtlabs-com/p-net.git"

    # Clone p-net
    if ! git clone --depth 1 "$pnet_repo" "$pnet_dir" 2>/dev/null; then
        log_error "Failed to clone p-net repository"
        return 1
    fi

    # Build p-net
    mkdir -p "$pnet_dir/build"
    if ! (cd "$pnet_dir/build" && cmake .. -DCMAKE_BUILD_TYPE=Release >/dev/null 2>&1); then
        log_error "p-net cmake configuration failed"
        rm -rf "$pnet_dir"
        return 1
    fi
    ...
    if ! (cd "$pnet_dir/build" && make -j"$nproc_count" >/dev/null 2>&1); then
        log_error "p-net build failed"
        rm -rf "$pnet_dir"
        return 1
    fi

    # Install p-net
    if ! (cd "$pnet_dir/build" && run_privileged make install >/dev/null 2>&1); then
    ...
    log_info "p-net PROFINET stack installed successfully"
    return 0
}
