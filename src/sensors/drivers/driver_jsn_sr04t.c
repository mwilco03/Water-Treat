...
result_t jsn_sr04t_init(jsn_sr04t_device_t *dev, int trigger_pin, int echo_pin) {
    ...
    result_t result = hwif_gpio_export(&dev->trigger, trigger_pin);
    if (result != RESULT_OK) {
        LOG_ERROR("Failed to export JSN-SR04T trigger pin %d", trigger_pin);
        return result;
    }
    hwif_gpio_set_direction(&dev->trigger, GPIO_DIR_OUT);
    hwif_gpio_write(&dev->trigger, false);

    // Export echo pin (input)
    result = hwif_gpio_export(&dev->echo, echo_pin);
    if (result != RESULT_OK) {
        LOG_ERROR("Failed to export JSN-SR04T echo pin %d", echo_pin);
        hwif_gpio_unexport(&dev->trigger);
        return result;
    }
    hwif_gpio_set_direction(&dev->echo, GPIO_DIR_IN);
...
result_t jsn_sr04t_read_distance_cm(jsn_sr04t_device_t *dev, float *distance_cm) {
    ...
    hwif_gpio_write(&dev->trigger, true);
    usleep(10);
    hwif_gpio_write(&dev->trigger, false);
    ...
    while (timeout_counter < max_timeout) {
        hwif_gpio_read(&dev->echo, &echo_state);
        if (echo_state) {
            break;
        }
        usleep(1);
