...
result_t solenoid_init(solenoid_valve_t *valve, int gpio_pin, bool inverted) {
    result_t result = gpio_export(&valve->gpio, gpio_pin);
    if (result != RESULT_OK) {
        return result;
    }
    
    gpio_set_direction(&valve->gpio, GPIO_DIR_OUT);
    
    valve->inverted = inverted;
    valve->is_open = false;
    
    // Start closed
    gpio_write(&valve->gpio, inverted ? true : false);
...
void solenoid_destroy(solenoid_valve_t *valve) {
    ...
    gpio_unexport(&valve->gpio);
}

result_t solenoid_open(solenoid_valve_t *valve) {
    result_t result = gpio_write(&valve->gpio, valve->inverted ? false : true);
...
result_t solenoid_close(solenoid_valve_t *valve) {
    result_t result = gpio_write(&valve->gpio, valve->inverted ? true : false);
