...
result_t pump_init(water_pump_t *pump, int gpio_pin, bool use_pwm) {
    ...
    result_t result = gpio_export(&pump->gpio, gpio_pin);
    if (result != RESULT_OK) {
        return result;
    }
    
    gpio_set_direction(&pump->gpio, GPIO_DIR_OUT);
    gpio_write(&pump->gpio, false);  // Start off
...
void pump_destroy(water_pump_t *pump) {
    pump_stop(pump);
    gpio_unexport(&pump->gpio);
}

result_t pump_start(water_pump_t *pump) {
    result_t result = gpio_write(&pump->gpio, true);
...
result_t pump_stop(water_pump_t *pump) {
    result_t result = gpio_write(&pump->gpio, false);
